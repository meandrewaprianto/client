...
export default function ProductDetails() {
    ...
    if (loading) {
        return (
            <SafeAreaView className='flex-1 justify-center items-centerw'>
            ...
            <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
                ...
                <View className='px-5'>
                    ...
                    {product.sizes && product.sizes.length > 0 && (
                        <>
                            <Text className='text-base font-bold text-primary mb-2'>Size</Text>
                            <View className='flex-row gap-3 mb-6 flex-wrap'>
                                {product.sizes.map((size) => (
                                    <TouchableOpacity
                                        key={size}
                                        onPress={() => setSelectedSize(size)}
                                        className={`w-12 h-12 rounded-full items-center justify-center border ${selectedSize === size ? 'border-primary bg-primary' : 'bg-white border-gray-300'}`}>
